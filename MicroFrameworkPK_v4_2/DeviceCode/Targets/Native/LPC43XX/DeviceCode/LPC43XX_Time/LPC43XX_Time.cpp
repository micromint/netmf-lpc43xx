...
#define SYSTIMER          ((LPC_TIMER_T *)LPC_TIMER3_BASE)
...
UINT64 systimer_counter(void);
	
BOOL HAL_Time_Initialize()
{
    g_lastCount = 0;
    g_nextEvent = 0xFFFFFFFFFFFFFFFF;
...
// Since this is probably calibrated for RAM execution and it's used by
// the FLASH routines, make sure to have this function in RAM...
#pragma arm section code = "SectionForFlashOperations"

//
// To calibrate this constant, uncomment #define CALIBRATE_SLEEP_USEC in TinyHAL.c
//
#define _SLEEP_USEC_FIXED_OVERHEAD_CLOCKS 3
...
void __section(SectionForFlashOperations) HAL_Time_Sleep_MicroSeconds( UINT32 uSec )
{
    ...
    if(maxDiff <= _SLEEP_USEC_FIXED_OVERHEAD_CLOCKS) maxDiff = 0; 
    else maxDiff -= _SLEEP_USEC_FIXED_OVERHEAD_CLOCKS;  // Subtract overhead
...
void __section(SectionForFlashOperations) HAL_Time_Sleep_MicroSeconds_InterruptEnabled( UINT32 uSec )
{
...
void __section(SectionForFlashOperations) systimer_handler(void* param)
{
    GLOBAL_LOCK(irq);
	
    SYSTIMER->IR = 1; // Clear interrupt

    if (systimer_counter() >= g_nextEvent) { // Past event time?
...
UINT64 __section(SectionForFlashOperations) systimer_counter(void)
{
